...
public class TypeParser {
    protected static TypeStats parseType(List<String> lines, Type type) {
        lines = StringHelper.cleanAllStrings(lines);
        return switch (type) {
            case EARTH -> new EarthStats();
            case ELECTRIC -> {
                ...
                yield new ElectricStats(paralysis_probability);
            }
            case FIRE -> {
                ...
                yield new FireStats(fire_probability);
            }
            case NATURE -> new NatureStats();
            case GRASS -> {
                ...
                yield new GrassStats(heal_probability);
            }
            case INSECT -> new InsectStats();
            case WATER -> {
                ...
                yield new WaterStats(flooding_probability, falling_probability);
            }
